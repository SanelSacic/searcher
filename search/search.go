...
// ContentHTML fixes ecoding issues.
func (r *Result) ContentHTML() template.HTML {
	return template.HTML(r.Content)
}

// Searcher declares an interface used to levrage different
...
func Submit(uid string, options Options) []Result {
	...
	// Create a BBC Searcher if checked.s
	if options.BBC {
	...
	for search := 0; search < len(searchers); search++ {
		...
		// Failing to do so will leave the Searcher blocked forever.

...
import (
	...
	"github.com/gocode/SearchForNews/search"
)

// req keeps track of the number of request.
...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	// If this is a post, perform search.
...
func render(fv map[string]interface{}, results []search.Result) []byte {
	...
	// Generate the final markuo with the layout template.
	vars := map[string]interface{}{"LayoutContent": template.HTML(string(markup))}
	return executeTemplate("layouts", vars)
}
